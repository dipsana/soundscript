...
- Seek‑bar sync and drag coordinate precision
- Hamburger menu pre‑load flicker (CSS‑only toggle)
- Play‑bar visibility flash on initial load
- Touch‑swipe direction detection in card‑slider
- Dynamic “Show All” button visibility (hidden when slider shifted)
- Loading system race conditions (manual show/hide in `gen‑util`)
- UI state sync in `music‑manager`, `ui‑util`, and `nav`

**Refactored:**

- Replaced fragile `DOMContentLoaded` loader with manual control
- Added `hideSect`/`showSect` for instant, reliable navigation
- Removed deprecated `loadingPage` function
- Cleaned up event listener dependencies

**Enhanced:**

- Smoather mobile gesture handling
- Consistent UI feedback across interactions
- More reliable SPA‑style navigation
